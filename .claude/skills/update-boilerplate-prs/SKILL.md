...
3. **Only then enable auto-merge.** Once step 1's non-watch output shows only `pass` / `skipping`, check `gh pr view <number> -R fohte/<repo> --json isDraft -q .isDraft` first -- PRs created by `copier-update-action` with unresolved conflicts start as draft, and `gh pr merge` fails on a draft PR with `GraphQL: Pull Request is still a draft (mergePullRequest)`. If `isDraft` is `true`, run `gh pr ready <number> -R fohte/<repo>` before merging. Then run `gh pr merge <number> -R fohte/<repo> --squash --auto`
